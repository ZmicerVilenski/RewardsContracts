...
import {Rewards} from "./Rewards.sol"; // Aqualis rewards
...
contract Pool is VersionedInitializable, PoolStorage, IPool {
    ...
    // Aqualis rewards begin
    bool internal lendingRewardsActive;
    mapping(address => address) internal _rewardContracts; 

    function setLendingRewardFlag(bool flag) onlyPoolAdmin external {
        lendingRewardsActive = flag;
    }

    function setRewardContractsAddresses(address asset, address rewardContract) onlyPoolAdmin external {
        _rewardContracts[asset] = rewardContract;
    }
    // Aqualis rewards end
    ...
    function supply(
        ...
        // Aqualis rewards
        // For every asset will be separate contract    
        if (lendingRewardsActive) {    
            if (_rewardContracts[asset] != address(0x0)) {
                Rewards(_rewardContracts[asset]).supply(onBehalfOf, amount);
            }
        }
    }
    ...
    function withdraw(
        ...
        if (lendingRewardsActive) {    
            if (_rewardContracts[asset] != address(0x0)) {
                Rewards(_rewardContracts[asset]).withdraw(to, amount);
            }
        }
        return
    ...
    function borrow(
        ...
        if (lendingRewardsActive) {    
            if (_rewardContracts[asset] != address(0x0)) {
                Rewards(_rewardContracts[asset]).borrow(onBehalfOf, amount);
            }
        }
    }
    ...
    function repay(
        ...
        if (lendingRewardsActive) {    
            if (_rewardContracts[asset] != address(0x0)) {
                Rewards(_rewardContracts[asset]).repay(onBehalfOf, amount);
            }
        }
        return
    ...
    function repayWithATokens(
        ...
        if (lendingRewardsActive) {    
            if (_rewardContracts[asset] != address(0x0)) {
                Rewards(_rewardContracts[asset]).repay(msg.sender, amount);
            }
        }
        return
            BorrowLogic.executeRepay(
